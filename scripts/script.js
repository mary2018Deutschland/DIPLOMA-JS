...
function createEvent(arr) {
  arr.forEach((eventElement) => {
    ...
    if (eventElement.type === "online") {
      ...
      smallOnlineEventDiv.append(
        smallCameraImage,
        onlineEventText.cloneNode(true)
      ); // Клонируем текст для второго div
...
function filterEvents(arr) {
  const selectedType =
    eventTypeSelect.value === "all" ? undefined : eventTypeSelect.value;
  const selectedDistance =
    eventDistanceSelect.value === "any" ? undefined : eventDistanceSelect.value;
  const selectedCategory =
    eventCategorySelect.value === "any" ? undefined : eventCategorySelect.value;
  let filteredArr = arr;
  ...
  if (selectedDistance) {
    filteredArr = filteredArr.filter(
      (element) => String(element.distance) === selectedDistance
    );
  }
  if (selectedCategory) {
    filteredArr = filteredArr.filter(
